# 📚 Temario Maestro Felinode: Ruta de Aprendizaje Integrada

Este documento combina la **didáctica educativa** del currículo mexicano con la **completitud técnica** del temario basado en el estándar Murphy (A1 a C2).

---

## 🎯 Metas por Etapa (Estándar Internacional)

| Etapa Educativa | Nivel MCER Meta | Enfoque Principal |
| :--- | :--- | :--- |
| **Secundaria (Básico)** | A1 - A2+ | Fundamentos, rutinas y pasado simple. |
| **Preparatoria (Intermedio)** | B1 - B2 | Fluidez, condicionales y voz pasiva. |
| **Universidad (Avanzado)** | C1 - C2 | Matices técnicos, inglés académico y maestría. |

---

## 🛠️ FASE 1: LOS FUNDAMENTOS (Didáctica de Secundaria)
*Nivel A1 a A2+ - El núcleo de la comunicación cotidiana.*

### **Primer Año (Nivel A1 - Elemental)**
**Meta**: Presentarse, describir el entorno y hablar en presente.

1. **Unidad 1: Identidad y Entorno (A1)**
    - 1.1 Saludos y Presentaciones [✅ Ya en DB]
    - 1.2 Verbo To Be y Pronombres [✅ Ya en DB]
    - 1.3 Artículos y Plurales [✅ Ya en DB]
    - 1.4 Adjetivos Demostrativos [✅ Ya en DB]
    - 1.5 Adjetivos Posesivos [✅ Ya en DB]
    - 1.6 There is / There are [✅ Ya en DB]
    - 1.7 Preposiciones de Lugar [✅ Ya en DB]
    - 1.8 Adjetivos y Sustantivos [✅ Ya en DB]
    - 1.9 Adjetivos Descriptivos [✅ Ya en DB]
    - 1.10 Adjetivos Numerales [✅ Ya en DB]

2. **Unidad 2: Vida Diaria y Rutinas (A1)**
    - 2.1 Presente Simple: Afirmativo [✅ Ya en DB]
    - 2.2 Presente Simple: Negativo [✅ Ya en DB]
    - 2.3 Presente Simple: Preguntas [✅ Ya en DB]
    - 2.4 Wh-Questions [✅ Ya en DB]
    - 2.5 Pronombres Objeto [✅ Ya en DB]
    - 2.6 Adverbios de Frecuencia [✅ Ya en DB]
    - 2.7 Preposiciones de Tiempo [✅ Ya en DB]
    - 2.8 Trabajo y Cualidades [✅ Ya en DB]

3. **Unidad 3: Acciones en Progreso y Habilidades (A1-A2)**
    - 3.1 Presente Continuo [✅ Ya en DB]
    - 3.2 Verbos Estáticos [✅ Ya en DB]
    - 3.3 Adjetivos Indefinidos [✅ Ya en DB]
    - 3.4 Cuantificadores [✅ Ya en DB]
    - 3.5 Sustantivos Incontables "Trampa" (Advice, Furniture, etc.) [✅ Ya en DB]
    - 3.6 Cuantificadores Específicos: Each, Every, Both, etc. [✅ Ya en DB]
    - 3.7 Can / Can't (Habilidad y permiso)[✅ Ya en DB]
    - 3.8 Imperativos (Órdenes e instrucciones)[✅ Ya en DB]

### **Segundo Año (Nivel A2 - Pre-Intermedio)**
**Meta**: Narrar historias, comparar el mundo y hacer planes.

4. **Unidad 4: Narración y Pasado (A2)**
    - 4.1 Verbo To Be: Pasado [✅ Ya en DB]
    - 4.2 There was / There were [✅ Ya en DB]
    - 4.3 Verbos Regulares (Pasado Simple) [✅ Ya en DB]
    - 4.4 Verbos Irregulares en Pasado Simple [✅ Ya en DB]
    - 4.5 Pasado Simple [✅ Ya en DB]
    - 4.6 El Pasado Continuo (Past Continuous) [✅ Ya en DB]
    - 4.7 Conectores (and, but, because, so, or) [✅ Ya en DB]

5. **Unidad 5: Comparaciones y Futuro (A2)**
    - 5.1 Comparaciones de Igualdad: As... as (Tan... como) [✅ Ya en DB]
    - 5.2 Comparativos y Superlativos en Inglés [✅ Ya en DB]
    - 5.3 Futuro con GOING TO [✅ Ya en DB]
    - 5.4 El Futuro Simple con "WILL" [✅ Ya en DB]

6. **Unidad 6: Pronombres Avanzados y Modales (A2)**
    - 6.1 Pronombres objeto [✅ Ya en DB]
    - 6.2 Pronombres Posesivos [✅ Ya en DB]
    - 6.3 Adjetivos posesivos vs Pronombres posesivos [✅ Ya en DB]
    - 6.4 Pronombres Reflexivos [✅ Ya en DB]
    - 6.5 Adjetivos Posesivos vs. Pronombres Reflexivos [✅ Ya en DB]
    - 6.6 Modales de Permiso y Petición (Can, could, may, would) [✅ Ya en DB]
    - 6.7 Must / Have to (Obligación y necesidad)[✅ Ya en DB]
    - 6.8 Consejos y Crítica (Should, ought to, had better)[✅ Ya en DB]
    - 6.9 Modales de Probabilidad y Deducción (May, might, could, must be, can't be)[✅ Ya en DB]
    - 6.10 Preposiciones de Movimiento (Into, onto, through, across, past)[✅ Ya en DB]
    - 6.11 Adjetivo + Preposición (Interested in, proud of, good at)[✅ Ya en DB]
    - 6.12 Verbo + Preposición (Listen to, depend on, wait for, etc)[✅ Ya en DB]

### **Tercer Año (Nivel A2+ a B1 - Umbral)**
**Meta**: Conectar experiencias pasadas con el presente y condiciones básicas.

7. **Unidad 7: Experiencias y Resultados (B1)**
    - 7.1 Been vs. Gone[✅ Ya en DB]
    - 7.2 Just, already, yet
    - 7.3 For y Since (Duración)
    - 7.4 Contraste: Presente Perfecto vs. Pasado Simple
    - 7.5 Presente Perfecto [✅ Ya en DB]

8. **Unidad 8: Estructuras Avanzadas de Secundaria (B1)**
    - 8.1 First Conditional (If + present, will + infinitive)
    - 8.2 Used to (Hábitos pasados)
    - 8.3 Voz Pasiva Básica (Presente y Pasado Simple)
    - 8.4 Relative Clauses básicos (who, which, that)
    - 8.5 Too y Enough
    - 8.6 Phrasal Verbs: Separables vs. Inseparables
    - 8.7 Phrasal Verbs de tres palabras (e.g., look forward to)

---

## 🚀 FASE 2: CONSOLIDACIÓN (Didáctica de Preparatoria)
*Nivel B1+ a B2 - Especialización y Fluidez.*

9. **Unidad 9: Tiempos Perfectos y Futuros Avanzados (B1+)**
    - 9.1 Presente Perfecto Continuo (have been doing)
    - 9.2 Pasado Perfecto (had done) vs. Pasado Simple
    - 9.3 Futuro Continuo (will be doing) y Futuro Perfecto (will have done)
    - 9.4 Future with Present Continuous and Present Simple

10. **Unidad 10: Hipótesis y Deseos (B2)**
    - 10.1 Second Conditional (If + past, would + infinitive)
    - 10.2 Third Conditional (If + past perfect, would have + participle)
    - 10.3 Mixed Conditionals
    - 10.4 I wish / If only (Presente, pasado y futuro)

11. **Unidad 11: Voz Pasiva y Reporte (B2)**
    - 11.1 Voz Pasiva en todos los tiempos
    - 11.2 Causative: Have/Get something done
    - 11.3 Reported Speech: Statements, questions and commands
    - 11.4 Reporting Verbs (suggest, advise, warn)

12. **Unidad 12: Estilo y Cohesión (B2+)**
    - 12.1 Verbos seguidos de -ing (enjoy, suggest)
    - 12.2 Verbos seguidos de to + infinitive (decide, hope)
    - 12.3 Verbos que cambian de significado (stop, remember, forget, try)
    - 12.4 Gerundios e Infinitivos avanzados (verbo + objeto + inf)
    - 12.5 Modal Verbs de deducción pasada (must have, might have, can't have)
    - 12.6 Relative Clauses: Defining vs. Non-defining
    - 12.7 Conectores de contraste y adición (however, moreover, notwithstanding)
    - 12.8 Cleft Sentences e Inversión básica (What I need is..., Never have I...)

---

## 🏛️ FASE 3: MAESTRÍA (Didáctica Universitaria)
*Nivel C1 a C2 - Inglés Académico y Profesional.*

13. **Unidad 13: Matices y Estructuras Complejas (C1)**
    - 13.1 Future Perfect Continuous
    - 13.2 Subjuntivo en inglés formal (I suggest he go)
    - 13.3 Inversión avanzada después de expresiones negativas y "only"
    - 13.4 Participle Clauses (-ing y -ed clauses)
    - 13.5 Nominal Relative Clauses (whoever, whatever, whichever)

14. **Unidad 14: Énfasis, Estilo y Retórica (C1-C2)**
    - 14.1 Fronting para énfasis y estructuras retóricas
    - 14.2 Nominalización y reducción de cláusulas
    - 14.3 Cohesión textual avanzada (Referencia anafórica y catafórica)
    - 14.4 Marcadores discursivos sofisticados (albeit, whereas)
    - 14.5 Puntuación avanzada para ritmo y estilo

15. **Unidad 15: Inglés Especializado (C2)**
    - 15.1 Idioms avanzados y Collocations académicas
    - 15.2 Registro formal vs. informal: Matices profundos
    - 15.3 Lenguaje legal, médico y periodístico
    - 15.4 Pragmática y sociolingüística (variantes globales del inglés)

---

## 📊 Evaluación para FeliNode (Roadmap de Implementación)

### **MÍNIMO VIABLE (Nivel Competitivo)**
✅ **A1 completo**: Verbo TO BE, Presente Simple, Pronombres, Preposiciones básicas.
✅ **A2 completo**: Pasado Simple, Comparativos, Futuro Going To/Will.
✅ **B1 básico**: Presente Perfecto, Primer Condicional, Phrasal Verbs comunes.

### **ESTÁNDAR PREMIUM (Aspiración México)**
✅ **A1 → B2 de forma fluida.**
✅ **Todos los condicionales (0, 1, 2, 3).**
✅ **Voz Pasiva y Reported Speech.**
✅ **Gerundios e Infinitivos avanzados.**

### **EXCELENTE (Diferenciador del Mercado)**
✅ **Contenido C1-C2**: Inversión, Subjuntivo, Inglés Académico.
✅ **Preparación para Certificaciones**: Cambridge, TOEFL, IELTS.
✅ **Análisis de variantes**: Diferencias entre inglés UK/US/AU.

---

## 🌍 Resumen Comparativo de Alcance

| Sistema | Bolivia/Chile | México | **FeliNode (Propuesto)** |
| :--- | :--- | :--- | :--- |
| **Meta Secundaria** | A2 | B1 | **A2+ / B1** |
| **Meta Prepa** | B1 | B2 | **B2 / B2+** |
| **Meta Universidad** | B1+ | B2+ | **C1 / C2** |
| **Total Lecciones** | ~60 | ~100 | **230 (Detalladas)** |
